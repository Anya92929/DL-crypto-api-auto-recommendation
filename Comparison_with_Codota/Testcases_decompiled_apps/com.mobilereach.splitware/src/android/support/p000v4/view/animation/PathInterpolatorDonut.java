...
class PathInterpolatorDonut implements Interpolator {
    ...
    public PathInterpolatorDonut(Path path) {
        ...
        this.f7mX = new float[numPoints];
        this.f8mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
            this.f7mX[i] = position[0];
            this.f8mY[i] = position[1];
        }
    }
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f7mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f7mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f7mX[endIndex] - this.f7mX[startIndex];
        if (xRange == 0.0f) {
            return this.f8mY[startIndex];
        }
        float startY = this.f8mY[startIndex];
        return ((this.f8mY[endIndex] - startY) * ((t - this.f7mX[startIndex]) / xRange)) + startY;
    }
