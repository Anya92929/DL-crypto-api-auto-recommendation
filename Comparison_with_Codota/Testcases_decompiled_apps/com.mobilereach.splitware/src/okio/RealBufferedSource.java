...
final class RealBufferedSource implements BufferedSource {
    ...
    public long readDecimalLong() throws IOException {
        int pos = 0;
        while (request((long) (pos + 1)) && (((b = this.buffer.getByte((long) pos)) >= 48 && b <= 57) || (pos == 0 && b == 45))) {
            pos++;
        }
        if (pos != 0) {
            return this.buffer.readDecimalLong();
        }
        throw new NumberFormatException("Expected leading [0-9] or '-' character but was 0x" + Integer.toHexString(this.buffer.getByte(0)));
    }

    public long readHexadecimalUnsignedLong() throws IOException {
        int pos = 0;
        while (request((long) (pos + 1)) && (((b = this.buffer.getByte((long) pos)) >= 48 && b <= 57) || ((b >= 97 && b <= 102) || (b >= 65 && b <= 70)))) {
            pos++;
        }
        if (pos != 0) {
            return this.buffer.readHexadecimalUnsignedLong();
        }
        throw new NumberFormatException("Expected leading [0-9a-fA-F] character but was 0x" + Integer.toHexString(this.buffer.getByte(0)));
    }
