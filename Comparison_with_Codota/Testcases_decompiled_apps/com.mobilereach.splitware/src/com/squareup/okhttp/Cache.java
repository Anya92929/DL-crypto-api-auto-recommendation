...
public final class Cache {
    ...
    private static final class Entry {
        ...
        private List<Certificate> readCertificateList(BufferedSource source) throws IOException {
            ...
            try {
                CertificateFactory certificateFactory = CertificateFactory.getInstance("X.509"); // CRYPTOGRAPHIC API CALLSITE 22
                List<Certificate> result = new ArrayList<>(length);
                for (int i = 0; i < length; i++) {
                    String line = source.readUtf8LineStrict();
                    Buffer bytes = new Buffer();
                    bytes.write(ByteString.decodeBase64(line));
                    result.add(certificateFactory.generateCertificate(bytes.inputStream())); // CRYPTOGRAPHIC API CALLSITE 21
                }
                return result;
        ...
        private void writeCertList(BufferedSink sink, List<Certificate> certificates) throws IOException {
            try {
                ...
                for (int i = 0; i < size; i++) {
                    sink.writeUtf8(ByteString.m20of(certificates.get(i).getEncoded()).base64()); // CRYPTOGRAPHIC API CALLSITE 8
                    sink.writeByte(10);
                }
