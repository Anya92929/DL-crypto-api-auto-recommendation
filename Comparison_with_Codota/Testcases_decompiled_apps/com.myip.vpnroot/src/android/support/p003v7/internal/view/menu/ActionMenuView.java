...
import android.support.p003v7.internal.widget.LinearLayoutICS;
...
public class ActionMenuView extends LinearLayoutICS implements MenuBuilder.ItemInvoker, MenuView {
    ...
    private void onMeasureExactFormat(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == 1073741824) {
            itemHeightSpec = View.MeasureSpec.makeMeasureSpec(heightSize - heightPadding, 1073741824);
        } else {
            itemHeightSpec = View.MeasureSpec.makeMeasureSpec(Math.min(this.mMaxItemHeight, heightSize - heightPadding), ExploreByTouchHelper.INVALID_ID);
        }
    ...
    static int measureChildForCells(View child, int cellSize, int cellsRemaining, int parentHeightMeasureSpec, int parentHeightPadding) {
        ...
        if (cellsRemaining > 0 && (!hasText || cellsRemaining >= 2)) {
            child.measure(View.MeasureSpec.makeMeasureSpec(cellSize * cellsRemaining, ExploreByTouchHelper.INVALID_ID), childHeightSpec);
