...
final class DecodeHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        if (this.running) {
            if (message.what == C0776R.C0777id.decode) {
                decode((byte[]) message.obj, message.arg1, message.arg2);
            } else if (message.what == C0776R.C0777id.quit) {
                this.running = false;
    ...
    private void decode(byte[] data, int width, int height) {
        ...
        if (this.activity.getResources().getConfiguration().orientation == 2) {
            source = this.activity.getCameraManager().buildLuminanceSource(data, width, height);
        } else {
            ...
            int tmp = width;
            source = this.activity.getCameraManager().buildLuminanceSource(rotatedData, height, tmp);
        }
        if (source != null) {
            try {
        ...
        if (rawResult != null) {
            ...
            if (handler != null) {
                Message message = Message.obtain(handler, C0776R.C0777id.decode_succeeded, rawResult);
            ...
            Message.obtain(handler, C0776R.C0777id.decode_failed).sendToTarget();
        }
    }
