...
final class LoadPackagesAsyncTask extends AsyncTask<Void, Void, List<AppInfo>> {
    ...
    public void onPostExecute(List<AppInfo> results) {
        ...
        this.activity.setListAdapter(new ArrayAdapter<AppInfo>(this.activity, C0776R.layout.app_picker_list_item, C0776R.C0777id.app_picker_list_item_label, results) {
            public View getView(int position, View convertView, ViewGroup parent) {
                View view = super.getView(position, convertView, parent);
                Drawable icon = ((AppInfo) list.get(position)).getIcon();
                if (icon != null) {
                    ((ImageView) view.findViewById(C0776R.C0777id.app_picker_list_item_icon)).setImageDrawable(icon);
                }
                return view;
            }
        });
