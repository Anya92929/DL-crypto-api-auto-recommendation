...
public final class ShareActivity extends Activity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        setContentView(C0776R.layout.share);
        findViewById(C0776R.C0777id.share_contact_button).setOnClickListener(this.contactListener);
        findViewById(C0776R.C0777id.share_bookmark_button).setOnClickListener(this.bookmarkListener);
        findViewById(C0776R.C0777id.share_app_button).setOnClickListener(this.appListener);
        this.clipboardButton = (Button) findViewById(C0776R.C0777id.share_clipboard_button);
        this.clipboardButton.setOnClickListener(this.clipboardListener);
        findViewById(C0776R.C0777id.share_text_view).setOnKeyListener(this.textListener);
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        if (resultCode == -1) {
            switch (requestCode) {
                case 0:
                case 2:
                    showTextAsBarcode(intent.getStringExtra(PlusShare.KEY_CALL_TO_ACTION_URL));
                    return;
                case 1:
                ...
                if (cursor != null) {
                    try {
                        if (cursor.moveToFirst()) {
                            String id = cursor.getString(cursor.getColumnIndex(DBFavorites.KEY_ID));
                            String name = cursor.getString(cursor.getColumnIndex("display_name"));
                            boolean hasPhone = cursor.getInt(cursor.getColumnIndex("has_phone_number")) > 0;
                            cursor.close();
                            Bundle bundle = new Bundle();
                            if (name != null && !name.isEmpty()) {
                                bundle.putString(DBFavorites.KEY_NAME, massageContactData(name));
                            }
    ...
    private static String massageContactData(String data) {
        ...
        if (data.indexOf(13) >= 0) {
            return data.replace("\r", MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR);
        }
        return data;
    }
}
