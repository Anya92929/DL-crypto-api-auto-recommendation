...
public final class Base64Variant implements Serializable {
    ...
    public String encode(byte[] input, boolean addQuotes) {
        ...
        while (inputPtr <= safeInputEnd) {
            ...
            encodeBase64Chunk(sb, (((input[inputPtr] << 8) | (input[inputPtr2] & MotionEventCompat.ACTION_MASK)) << 8) | (input[inputPtr3] & MotionEventCompat.ACTION_MASK));
            chunksBeforeLF--;
            if (chunksBeforeLF <= 0) {
                sb.append('\\');
                sb.append('n');
        ...
        if (inputLeft > 0) {
            ...
            if (inputLeft == 2) {
                b24 |= (input[inputPtr5] & Constants.UNKNOWN) << 8;
                int i = inputPtr5 + 1;
            }
