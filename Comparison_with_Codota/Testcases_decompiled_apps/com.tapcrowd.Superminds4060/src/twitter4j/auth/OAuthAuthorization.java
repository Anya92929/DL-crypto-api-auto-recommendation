...
public class OAuthAuthorization implements Authorization, Serializable, OAuthSupport {
    ...
    private static final HttpParameter OAUTH_SIGNATURE_METHOD = new HttpParameter(OAuthConstants.SIGN_METHOD, "HMAC-SHA1");
    ...
    public RequestToken getOAuthRequestToken(String callbackURL, String xAuthAccessType) throws TwitterException {
        ...
        if (callbackURL != null) {
            params.add(new HttpParameter(OAuthConstants.CALLBACK, callbackURL));
        }
    ...
    public String generateAuthorizationHeader(String method, String url, HttpParameter[] params, String nonce, String timestamp, OAuthToken otoken) {
        ...
        oauthHeaderParams.add(new HttpParameter(OAuthConstants.CONSUMER_KEY, this.consumerKey));
        oauthHeaderParams.add(OAUTH_SIGNATURE_METHOD);
        oauthHeaderParams.add(new HttpParameter(OAuthConstants.TIMESTAMP, timestamp));
        oauthHeaderParams.add(new HttpParameter(OAuthConstants.NONCE, nonce));
        oauthHeaderParams.add(new HttpParameter(OAuthConstants.VERSION, "1.0"));
        if (otoken != null) {
        ...
        oauthHeaderParams.add(new HttpParameter(OAuthConstants.SIGNATURE, signature));
        if (this.realm != null) {
    ...
    public List<HttpParameter> generateOAuthSignatureHttpParams(String method, String url) {
        ...
        StringBuilder base = new StringBuilder(method).append("&").append(HttpParameter.encode(constructRequestURL(url))).append("&");
        base.append(HttpParameter.encode(normalizeRequestParameters(signatureBaseParams)));
        oauthHeaderParams.add(new HttpParameter(OAuthConstants.SIGNATURE, generateSignature(base.toString(), this.oauthToken)));
    ...
    public String generateSignature(String data, OAuthToken token) {
        SecretKeySpec spec;
        try {
            Mac mac = Mac.getInstance(HMAC_SHA1); // CRYPTOGRAPHIC API CALLSITE 13
            if (token == null) {
                spec = new SecretKeySpec((HttpParameter.encode(this.consumerSecret) + "&").getBytes(), HMAC_SHA1); // CRYPTOGRAPHIC API CALLSITE 2
            } else {
                spec = token.getSecretKeySpec();
                if (spec == null) {
                    spec = new SecretKeySpec((HttpParameter.encode(this.consumerSecret) + "&" + HttpParameter.encode(token.getTokenSecret())).getBytes(), HMAC_SHA1); // CRYPTOGRAPHIC API CALLSITE 3
                    token.setSecretKeySpec(spec);
                }
            }
            mac.init(spec); // CRYPTOGRAPHIC API CALLSITE 11
            return BASE64Encoder.encode(mac.doFinal(data.getBytes())); // CRYPTOGRAPHIC API CALLSITE 4
