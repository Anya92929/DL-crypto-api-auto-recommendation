...
class TwitterImpl extends TwitterBaseImpl implements Twitter {
    ...
    public Status updateStatus(String status) throws TwitterException {
        return this.factory.createStatus(post(this.conf.getRestBaseURL() + "statuses/update.json", new HttpParameter[]{new HttpParameter("status", status)}));
    }
    ...
    public User updateProfile(String name, String url, String location, String description) throws TwitterException {
        ...
        addParameterToList(profile, DBFavorites.KEY_NAME, name);
        addParameterToList(profile, PlusShare.KEY_CALL_TO_ACTION_URL, url);
        addParameterToList(profile, "location", location);
        addParameterToList(profile, PlusShare.KEY_CONTENT_DEEP_LINK_METADATA_DESCRIPTION, description);
    ...
    public ResponseList<User> searchUsers(String query, int page) throws TwitterException {
        return this.factory.createUserList(get(this.conf.getRestBaseURL() + "users/search.json", new HttpParameter[]{new HttpParameter("q", query), new HttpParameter("per_page", 20), new HttpParameter("page", page)}));
    }
    ...
    public void updateProfileBanner(File image) throws TwitterException {
        checkFileValidity(image);
        post(this.conf.getRestBaseURL() + "account/update_profile_banner.json", new HttpParameter[]{new HttpParameter("banner", image)});
    }

    public void updateProfileBanner(InputStream image) throws TwitterException {
        post(this.conf.getRestBaseURL() + "account/update_profile_banner.json", new HttpParameter[]{new HttpParameter("banner", "banner", image)});
    }
    ...
    private UserList updateUserList(String newListName, boolean isPublicList, String newDescription, HttpParameter... params) throws TwitterException {
        ...
        if (newListName != null) {
            httpParams.add(new HttpParameter(DBFavorites.KEY_NAME, newListName));
        }
        httpParams.add(new HttpParameter("mode", isPublicList ? "public" : "private"));
        if (newDescription != null) {
            httpParams.add(new HttpParameter(PlusShare.KEY_CONTENT_DEEP_LINK_METADATA_DESCRIPTION, newDescription));
        }
    ...
    public UserList createUserList(String listName, boolean isPublicList, String description) throws TwitterException {
        List<HttpParameter> httpParams = new ArrayList<>();
        httpParams.add(new HttpParameter(DBFavorites.KEY_NAME, listName));
        httpParams.add(new HttpParameter("mode", isPublicList ? "public" : "private"));
        if (description != null) {
            httpParams.add(new HttpParameter(PlusShare.KEY_CONTENT_DEEP_LINK_METADATA_DESCRIPTION, description));
        }
    ...
    public SimilarPlaces getSimilarPlaces(GeoLocation location, String name, String containedWithin, String streetAddress) throws TwitterException {
        ...
        params.add(new HttpParameter(Globalization.LONG, location.getLongitude()));
        params.add(new HttpParameter(DBFavorites.KEY_NAME, name));
    ...
    public ResponseList<Location> getClosestTrends(GeoLocation location) throws TwitterException {
        return this.factory.createLocationList(get(this.conf.getRestBaseURL() + "trends/closest.json", new HttpParameter[]{new HttpParameter("lat", location.getLatitude()), new HttpParameter((String) Globalization.LONG, location.getLongitude())}));
    }
