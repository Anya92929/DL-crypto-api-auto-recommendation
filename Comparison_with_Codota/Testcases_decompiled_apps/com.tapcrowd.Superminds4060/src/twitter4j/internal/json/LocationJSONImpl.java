    ...
    LocationJSONImpl(JSONObject location) throws TwitterException {
        try {
            ...
            if (!location.isNull("placeType")) {
                ...
                this.placeName = z_T4JInternalParseUtil.getUnescapedString(DBFavorites.KEY_NAME, placeJSON);
                this.placeCode = z_T4JInternalParseUtil.getInt(OAuthConstants.CODE, placeJSON);
            } else {
                this.placeName = null;
                this.placeCode = -1;
            }
            this.name = z_T4JInternalParseUtil.getUnescapedString(DBFavorites.KEY_NAME, location);
            this.url = z_T4JInternalParseUtil.getUnescapedString(PlusShare.KEY_CALL_TO_ACTION_URL, location);
