...
public class NewsHeaderListAdapter extends ArrayAdapter<Post> {
    private ImageLoadingListener animateFirstListener = new AnimateFirstDisplayListener((AnimateFirstDisplayListener) null);
    ...
    private class ViewHolder {
        ...
        /* synthetic */ ViewHolder(NewsHeaderListAdapter newsHeaderListAdapter, ViewHolder viewHolder) {
            this();
        }
    }

    public NewsHeaderListAdapter(Context ctx, int resourceId, ArrayList<Post> objects, DisplayImageOptions options2, int category2) {
    ...
    public NewsHeaderListAdapter(Context ctx, int resourceId, ArrayList<Post> objects, DisplayImageOptions options2, int category2, boolean isSavedArticles2) {
        super(ctx, resourceId, objects);
        this.resource = resourceId;
        this.inflater = LayoutInflater.from(ctx);
        this.context = ctx;
    ...
    private void setImage(int position, ViewHolder holder) {
        if (this.items.get(position).getThumbnail() == null || this.items.get(position).getThumbnail().equals("null")) {
            ...
            this.imageLoader.displayImage(this.items.get(position).getThumbnail(), holder.image, this.options, this.animateFirstListener);
        }
    }
