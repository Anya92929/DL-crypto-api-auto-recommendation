...
public class MenuBuilder implements Menu {
    ...
    public void restoreActionViewStates(Bundle states) {
        ...
        if (states != null) {
            ...
            if (Build.VERSION.SDK_INT >= 11 || viewStates != null) {
                int itemCount = size();
                for (int i = 0; i < itemCount; i++) {
                    MenuItem item = getItem(i);
                    View v = item.getActionView();
    ...
    private void setShortcutsVisibleInner(boolean shortcutsVisible) {
        boolean z = true;
        if (!shortcutsVisible || this.mResources.getConfiguration().keyboard == 1 || !this.mResources.getBoolean(C0044R.bool.abs__config_showMenuShortcutsWhenKeyboardPresent)) {
            z = false;
        }
    ...
    public boolean bindNativeOverflow(android.view.Menu menu, MenuItem.OnMenuItemClickListener listener, HashMap<android.view.MenuItem, MenuItemImpl> map) {
        ...
        for (MenuItemImpl nonActionItem : nonActionItems) {
            if (nonActionItem.isVisible()) {
                ...
                if (nonActionItem.hasSubMenu()) {
                    ...
                    Iterator<MenuItemImpl> it = ((SubMenuBuilder) nonActionItem.getSubMenu()).getVisibleItems().iterator();
                    while (it.hasNext()) {
                        MenuItemImpl subItem = it.next();
