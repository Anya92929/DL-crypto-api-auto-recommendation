...
public class Tracker {
    ...
    /* renamed from: DF */
    private static final DecimalFormat f753DF = new DecimalFormat("0.######", new DecimalFormatSymbols(Locale.US));
    ...
    public void sendView() {
        ...
        internalSend(ModelFields.APP_VIEW, (Map<String, String>) null);
    }
    ...
    public void sendEvent(String category, String action, String label, Long value) {
        ...
        internalSend(ModelFields.EVENT, constructEvent(category, action, label, value));
    ...
    public void sendException(String threadName, Throwable exception, boolean fatal) {
        ...
        if (this.mExceptionParser != null) {
            ...
            try {
                ...
                Log.m529w("trackException: couldn't serialize, sending \"Unknown Exception\"");
    ...
    private void internalSend(String hitType, Map<String, String> params) {
        ...
        this.mModel.setAll(params, true);
        if (!tokensAvailable()) {
    ...
    public void setCustomDimension(int index, String value) {
        if (index < 1) {
            Log.m529w("index must be > 0, ignoring setCustomDimension call for " + index + ", " + value);
        } else {
    ...
    public void setCustomMetric(int index, Long value) {
        if (index < 1) {
            Log.m529w("index must be > 0, ignoring setCustomMetric call for " + index + ", " + value);
            return;
        }
    ...
    public void setCustomDimensionsAndMetrics(Map<Integer, String> dimensions, Map<Integer, Long> metrics) {
        if (dimensions != null) {
            for (Integer key : dimensions.keySet()) {
                setCustomDimension(key.intValue(), dimensions.get(key));
            }
        }
        if (metrics != null) {
            for (Integer key2 : metrics.keySet()) {
                setCustomMetric(key2.intValue(), metrics.get(key2));
            }
        }
    }
    ...
    private static String microsToCurrencyString(long currencyInMicros) {
        return f753DF.format(((double) currencyInMicros) / 1000000.0d);
    }
    ...
    private static class SimpleModel {
        ...
        public synchronized String get(String key) {
            String result;
            result = this.temporaryMap.get(key);
            if (result == null) {
                result = this.permanentMap.get(key);
            }
            return result;
        }
