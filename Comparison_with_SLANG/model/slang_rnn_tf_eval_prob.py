...
class Model:
    ...
    def eval_prob(self,save_checkpoints,test_input,batch_size,output_prob_save):
        ...
        with tf.Session() as sess:
            # restore model
            self.lstm_saver.restore(sess, tf.train.latest_checkpoint(save_checkpoints))
            print('successfully restore lstm model')
            with open(args.logfile,'a') as file:
                file.write(f"Resotre lstm model from checkpoint: {save_checkpoints}.\n")
            ...
            for  apps, callsites, test_inputs, test_label_len, test_labels in test_input.get_test_batches(batch_size):
                ...
                i_batch += 1
                if (i_batch % 10==0):
                    print("Eval for {} batch, accuracy: {}".format(i_batch,accur_test))
...
test_inputs = Input_fn(args.test_data)
...
rnn_model.eval_prob(args.save_checkpoints,test_inputs,args.batch_size,output_prob_save)
