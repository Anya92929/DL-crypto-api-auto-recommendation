...
class Model:
    def __init__(self,voca_size,n_embedding,embedding_checkpoint, logfile, normalized_embedding=False):
        ...
        if embedding_checkpoint:
            ...
            with open(self.logfile,'a') as f:
                ...
                f.write("===> Normalized on embedding: {}\n".format(normalized_embedding))
        else:
            with open(logfile,'a') as f:
                f.write("===>No not resotre embedding.\n")
    ...
    def get_embedding_mat(self, embedding_checkpoint, normalized=False):
        ...
        with train_graph.as_default():
            ...
            softmax_w = tf.Variable(tf.truncated_normal((self.n_voca, self.n_embedding)))  # create softmax weight matrix here
    ...
    def build(self,lstm_units,use_embedding=False,update_next=False,max_step=10,regular_order_initial=True):
        with open(self.logfile,'a') as f:
            f.write("===> Build lstm_model with parameters:\n")
            f.write("[lstm_units: {}; use_embedding: {}; update_next: {}; max_step: {}, regular_order_initial: {}]\n"
                .format(lstm_units,use_embedding,update_next,max_step,regular_order_initial))
        #graph build        
        ...
        if regular_order_initial:
            self.weights=tf.Variable(tf.one_hot(position_id,max_step),name="weights")
        else:
            self.weights=tf.Variable(tf.truncated_normal([max_step,max_step]),name="weights")
        input_mat = tf.transpose(embedding_vec,[1,0,2])
        
        weighted_inputs=tf.tensordot(self.weights,input_mat,axes=[[1], [0]])
    ...
    def train_and_test(self,train_input,test_input,epoch,batch_size):
        ...
        with tf.Session() as sess:
            ...
            for e in range(epoch):
                ...
                self.lstm_saver.save(sess, "{}_{}.ckpt".format(save_model,e+1))
                print("====================Testing phase===================================")
    ...
    def test_by_length(self,lstm_checkpoint,test_input,batch_size,leng):
        ...
        #correct_recorder.close()
        #wrong_recorder.close()
    def test_for_length_range(self,lstm_checkpoint,test_input,batch_size,min_leng,max_leng):
        for i in range(min_leng,max_leng+1):
            with open(self.logfile,'a') as f:
                f.write("---------Length {}:{} samples-------------\n".format(i,len(test_input._data_by_length[i])))
            print("---------Length {}:{} samples-------------\n".format(i,len(test_input._data_by_length[i])))
            self.test_by_length(lstm_checkpoint,test_input,batch_size,i)

    def get_position_weight(self,checkpoint):
        with tf.Session() as sess:
            # restore model
            self.lstm_saver.restore(sess, tf.train.latest_checkpoint(checkpoint))
            with open(self.logfile,'a') as f:
                f.write(f'===> Restore lstm model from checkpoint: {checkpoint}\n')
