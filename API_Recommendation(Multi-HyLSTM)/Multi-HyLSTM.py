...
class Model:
    ...
    def build(self,lstm_units,use_embedding=True):
        ...
        self.recommend_logits= tf.nn.softmax(logits_ave)
    ...
    def test_for_A_app(self,multi_path_checkpoint,test_app_input,batch_size,app_name):
        log = open(self.logfile,'a')
        log.write("Test for App {}".format(app_name))
        logit_save = app_name[app_name.rindex('/')+1:app_name.index('_input_data.csv')] + 'logits.csv'
        logits_2d = []
        prediction_app = []
        with tf.Session() as sess:
            # restore model
            ...
            for test_inputs, test_label_len, test_n_path, test_labels in test_app_input.get_batches(batch_size):
                print(test_inputs)
                print(test_labels)
                accuracy, prediction, logits = sess.run([self.accuracy, self.prediction,self.recommend_logits], feed_dict={self.inputs:test_inputs,self.outputs:test_labels,self.target_len:test_label_len,self.n_path:test_n_path})
                i_test = len(test_labels)
                n_test += i_test
                n_correct += int(accuracy*i_test)
                prediction_app.extend(list(prediction)) 
                logits_2d.extend(list(logits))
                for pred in prediction:
                    log.write("Predict {}\t".format(pred))
            logits_2d = np.array(logits_2d)
            

            with open(logit_save,'w') as file:
                logit_writer = csv.writer(file)
                logit_writer.writerow(list(range(4543)))
                for logit in logits_2d:
                    logit_writer.writerow(list(logit))


            
                     
            acc_test = n_correct/n_test
            log.write("\nTest Accuracy for last token: {}\n".format(acc_test))
            print("Test Accuracy for last token: {}".format(acc_test))
        log.close()
