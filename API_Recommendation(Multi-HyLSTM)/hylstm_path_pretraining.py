...
parser.add_argument('--output_dir', dest='output_path', type=str)
parser.add_argument('--embedding_checkpoint_dir',dest='embedding_checkpoint_path',type=str)
parser.add_argument('--data_dir',dest='data_path',type=str)
...
parser.add_argument('--pretraining_data',dest='train_data',type=str)
...
with open(logfile,'w') as f:
    f.write("parameters:\n")
    f.write("[pretrain_data: {}; Use embedding: {}; Embedding_checkpoint: {}; update_next: {}; epoch: {}; batch_size: {}; save_checkpoint: {}]\n"
        .format(train_file,use_embedding,embedding_checkpoint,update_next,epoch,batch_size,save_checkpoint))

print("parameters:")
print("[train_data: {};  Use embedding: {}; Embedding_checkpoint: {}; update_next: {}; epoch: {}; batch_size: {}; save_checkpoint: {}]"
        .format(train_file, use_embedding,embedding_checkpoint,update_next,epoch,batch_size,save_checkpoint))
...
class Model:
    ...
    def build(self,lstm_units,use_embedding=False):
        ...
        softmax_weight_next = tf.Variable(tf.truncated_normal([lstm_units,self.n_voca]), name="softmax_weight_next")
        softmax_weight_last = tf.Variable(tf.truncated_normal([lstm_units,self.n_voca]),name = "softmax_weight_last")
        softmax_bias_next = tf.Variable(tf.constant(0.1, shape=[self.n_voca]))
        softmax_bias_last = tf.Variable(tf.constant(0.1, shape=[self.n_voca]))
        ...
        logits_next = tf.matmul(hidden_output,softmax_weight_next)+softmax_bias_next
        logits_next = tf.reshape(logits_next, [-1,n_time_step,self.n_voca])
        
        logits_last = tf.matmul(hidden_output,softmax_weight_last)+softmax_bias_last
        logits_last = tf.reshape(logits_last,[-1,n_time_step,self.n_voca])
        ...
        self.loss_next = tf.contrib.seq2seq.sequence_loss(
            logits_next,
            self.outputs,
            mask_for_padding,
        ...
        label_at_every_time_step = tf.boolean_mask(self.outputs,mask) # 1-D batch
        label_at_every_time_step = tf.expand_dims(label_at_every_time_step,axis=1) # 2-D: batch*1
        b = tf.stack([1,n_time_step]) 
        label_at_every_time_step = tf.tile(label_at_every_time_step,b)
        
        # only last token is used in the loss
        self.loss_last = tf.contrib.seq2seq.sequence_loss(
                    logits_last,
                    label_at_every_time_step,
                    mask,
                    average_across_timesteps=False,
                    average_across_batch=True) 
        ...
        #hybrid loss
        self.loss = self.loss_last+self.loss_next
        self.optimizer = tf.train.AdamOptimizer().minimize(self.loss)
        # answer from softmax_weights_next
        pred_next = tf.argmax(logits_next,2) #batch_size*time_step
        mask_pred = tf.multiply(tf.cast(pred_next,tf.float32),mask)
        # answer 
        self.pred1 = tf.reduce_sum(mask_pred,1)
        mask_output = tf.multiply(tf.cast(self.outputs,tf.float32),mask)
        # label
        last_output = tf.reduce_sum(mask_output,1)
        correctPred1 = tf.equal(tf.cast(self.pred1,tf.int64),tf.cast(last_output,tf.int64))
        self.next_acc = tf.reduce_mean(tf.cast(correctPred1,tf.float32))

        #answer from softmax_weights_last
        pred_last = tf.argmax(logits_last,2) #batch_size*time_step
        mask_pred_last = tf.multiply(tf.cast(pred_last,tf.float32),mask)
        # answer 
        self.pred2 = tf.reduce_sum(mask_pred_last,1)

        correctPred2 = tf.equal(tf.cast(self.pred2,tf.int64),tf.cast(last_output,tf.int64))
        self.last_acc = tf.reduce_mean(tf.cast(correctPred2,tf.float32))

        global_correct = tf.stack([correctPred1,correctPred2]) #2*batch
        global_correct = tf.reduce_max(tf.cast(global_correct,tf.int32),axis=0)
        self.global_acc = tf.reduce_mean(tf.cast(global_correct,tf.float32))
        ...
        with tf.Session() as sess:
            ...
            for e in range(epoch):
                ...
                for train_inputs,label_len, train_labels in train_input.get_batches(batch_size):
                    i_batch += 1
                    #print("Iteration:{}".format(batch_cnt))
                    _,los, accur,accur2, acc_global=sess.run([self.optimizer,self.loss,self.last_acc,self.next_acc,self.global_acc], feed_dict= {self.inputs:train_inputs, self.outputs:train_labels, self.target_len:label_len})
                    if i_batch % 100==0:
                        print("Epoch {}, Iteration {}/{}: Training accuracy from last_weight {}, from next_weight {}, from global {}; Training loss {}".format(e+1,i_batch,n_batch,accur,accur2,acc_global,los))
                        log.write("Epoch {}, Iteration {}/{}: Training accuracy from last_weight {}, from next_weight {}, from global {}; Training loss {}.\n".format(e+1,i_batch,n_batch,accur,accur2,acc_global,los))
                ...
                for test_inputs, test_label_len,test_labels in test_input.get_batches(batch_size):
                    ...
                    acc_next, acc_last, acc_global, los_test = sess.run([self.next_acc, self.last_acc, self.global_acc, self.loss], feed_dict={self.inputs:test_inputs,self.outputs:test_labels,self.target_len:test_label_len})
                    i_test = len(test_inputs)
                    acc_next_total += acc_next*i_test
                    acc_last_total += acc_last*i_test
                    acc_global_total += acc_global*i_test
                    n_test += i_test
                acc_next_ave = acc_next_total/n_test
                acc_last_ave = acc_last_total/n_test
                acc_global_ave = acc_global_total/n_test
              
                
                print("Test Accuracy after epoch {}: from next {}, from last {}, global {}; test loss: {}".format(e+1,acc_next_ave,acc_last_ave,acc_global_ave,los_test))
                log.write("Test Accuracy after epoch {}: from next {}, from last {}, global {}; test loss: {}\n".format(e+1,acc_next_ave,acc_last_ave,acc_global_ave,los_test))
        log.close()
...
lstm_model.sequence_embedding(train_input,epoch,batch_size,save_checkpoint)
